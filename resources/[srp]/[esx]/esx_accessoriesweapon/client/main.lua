...
AddEventHandler('eden_accesories:silencieux', function(duration)
    ...
    local currentWeaponHash = GetSelectedPedWeapon(ped)
		if used < silencieux then
			...
			if currentWeaponHash == GetHashKey("WEAPON_PISTOL") then
		  		 ...
		  		 ESX.ShowNotification(("~b~Założyłeś tłumik do broni.")) 
		  		 	used = used + 1
	...
	used = used + 1
		  		  ...
		  		  ESX.ShowNotification(("~o~Nie masz broni w ręku lub twoja broń nie może obsługiwać tłumika."))
		  		
			end
			else
					  		 ESX.ShowNotification(("~r~Ten tłumik jest już założony.")) 
...
AddEventHandler('eden_accesories:flashlight', function(duration)
    ...
    local currentWeaponHash = GetSelectedPedWeapon(ped)
		if used2 < flashlight then
			...
			if currentWeaponHash == GetHashKey("WEAPON_PISTOL") then
		  		 ...
		  		 ESX.ShowNotification(("~b~Założyłeś latarkę do broni.")) 
		  		 	used2 = used2 + 1
		  	elseif currentWeaponHash == GetHashKey("WEAPON_PISTOL50") then
	...
	used2 = used2 + 1
		  		  ...
		  		  ESX.ShowNotification(("~o~Nie masz broni w ręku lub twoja broń nie może mieć latarki."))
		  		
			end
		else
				  		  ESX.ShowNotification(("~r~Ta latarka jest już założona."))
...
AddEventHandler('eden_accesories:grip', function(duration)
    ...
    local currentWeaponHash = GetSelectedPedWeapon(ped)
		if used3 < grip then
			...
			if currentWeaponHash == GetHashKey("WEAPON_COMBATPDW") then
		  		 ...
		  		 ESX.ShowNotification(("~b~Założyłeś uchwyt do broni.")) 
		  				used3 = used3 + 1
	...
	used3 = used3 + 1
		  		  ...
		  		  ESX.ShowNotification(("~o~Nie masz broni w ręku lub twoja broń nie może mieć uchwytu."))
		  		
			end
		else
				  		  ESX.ShowNotification(("~rTen uchwyt jest już założony~."))
		end
end)
...
AddEventHandler('eden_accesories:yusuf', function(duration)
    ...
    local currentWeaponHash = GetSelectedPedWeapon(ped)
		if used4 < yusuf then
			...
			if currentWeaponHash == GetHashKey("WEAPON_PISTOL") then
		  		 ...
		  		 ESX.ShowNotification(("~b~Przemalowałeś broń.")) 
		  		 	used4 = used4 + 1
	...
	used4 = used4 + 1
		  		  ...
		  		  ESX.ShowNotification(("~o~Nie masz broni w ręku lub twoja broń nie może mieć zdobienia."))
		  		
			end
		else
				  		  ESX.ShowNotification(("~r~To zdobienie jest już wykorzystane."))
