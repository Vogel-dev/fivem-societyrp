		...
		if rob == false then
		  ...
		  if xPlayer.getInventoryItem('drill').count >= 1 then
			...
			if(cops >= Config.NumberOfCopsRequired)then
				...
				local message = "Gracz **"..PlayerName.."** rozpoczyna napad na bank **" .. bank.nameofbank .. "**."
        		BetonoweLogi("Napad na bank - rozpoczęty napad",PlayerName,message,xPlayer.identifier,0x9b4e00)
				...
				SetTimeout(6000000, function()
					...
					if(robbers[savedSource])then
						...
						if(xPlayer)then
							...
							local message = "Gracz **"..PlayerName.."** otrzymał **" .. bank.reward .. "**$ za udany napad na bank."
        					BetonoweLogi("Napad na bank - udany napad",PlayerName,message,xPlayer.identifier,0xff8000)
							local xPlayers = ESX.GetPlayers()
							for i=1, #xPlayers, 1 do
								local xPlayer = ESX.GetPlayerFromId(xPlayers[i])
								if xPlayer.job.name == 'police' then
