...
function JAM_Garage:CheckPosition()
    ...
    if standingInMarker and not self.StandingInMarker then
        ...
        self.ActionData.Message = 'Wciśnij ~INPUT_PICKUP~ aby otworzyć ~o~' .. (self.CurrentGarage.Zone:sub(1,1):lower()..self.CurrentGarage.Zone:sub(2)) .. '.'
    end
...
function JAM_Garage:OpenGarageMenu(zone)
    ...
    local elements = {}
    table.insert(elements,{label = "Lista pojazdów: " .. zone, value = zone .. "_List"})
    table.insert(elements,{label = "Schowaj pojazd: " .. zone, value = zone .. "_Vehicle"})
...
function JAM_Garage:OpenVehicleList(zone)
    ...
    ESX.TriggerServerCallback('JAM_Garage:GetVehicles', function(vehicles)
        for key,val in pairs(vehicles) do
            ...
            if val.state == 1 then
                labelvehicle = vehiclePlate .. " : " .. vehicleName .. " : Garaż"            
            elseif val.state == 2 then
                labelvehicle = vehiclePlate .. " : " .. vehicleName .. " : Ubezpieczalnia"      
            else                
                labelvehicle = vehiclePlate .. " : " .. vehicleName .. " : Na mapie"      
            end 
        ...
        function(data, menu)
            if zone == 'Garaż' then
                if data.current.value.state == 1 then
                    ...
                    TriggerEvent('esx:showNotification', '~r~Twojego pojazdu nie ma w garażu.')
                end
            end

            if zone == 'Ubezpieczalnia' then
                if data.current.value.state == 2 then
                    menu.close()
                    JAM_Garage:SpawnVehicle(data.current.value.vehicle)
                else
                    TriggerEvent('esx:showNotification', '~r~Twój pojazd nie jest w ubezpieczalni.')
                end
            end
        end,
        ...
        if not IsPedInVehicle(GetPlayerPed(), vehicle, false) then
            ESX.TriggerServerCallback('JAM_Garage:StoreVehicle', function(valid)
                if(valid) then
                    ...
                    TriggerEvent('esx:showNotification', '~g~Twój pojazd został schowany.')
                else
                    TriggerEvent('esx:showNotification', "~o~Ten pojazd nie jest twój.")
                end
            end, vehicleProps)
