		...
		if radar.shown  then
			if radar.freeze == false then
					...
					if IsEntityAVehicle(e) then
						...
						radar.info = string.format("~y~Tablica: ~w~%s  ~y~Model: ~w~%s  ~y~Prędkość: ~w~%s mph", fplate, fmodel, math.ceil(fvspeed))
					end
					...
					if IsEntityAVehicle(j) then
						...
						radar.info2 = string.format("~y~Tablica: ~w~%s  ~y~Model: ~w~%s  ~y~Prędkość: ~w~%s mph", bplate, bmodel, math.ceil(bvspeed))
					end					
			end
