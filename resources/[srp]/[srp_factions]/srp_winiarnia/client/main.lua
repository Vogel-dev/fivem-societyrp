...
function OpenVehicleSpawnerMenu()
	...
	if Config.EnableSocietyOwnedVehicles then
		...
		local elements = {
			{label = _U('Bison 3'),  value = 'bison3'}
		}

		if Config.EnablePlayerManagement and PlayerData.job ~= nil and (PlayerData.job.grade_name == 'cdisenior') then
			table.insert(elements, {label = 'Dodge Ram 3500', value = '193500hd'})
		end
...
AddEventHandler('esx_vigneronjob:hasEnteredMarker', function(zone)
	if zone == 'winogronoFarm' and PlayerData.job ~= nil and PlayerData.job.name == 'vigne'  then
		CurrentAction     = 'winogrono_harvest'
...
function blips()
	if publicBlip == false then
		...
		AddTextComponentString("Winiarnia")
	...
	while true do
		...
		if CurrentAction ~= nil then
			...
			if IsControlPressed(0,  Keys['E']) and PlayerData.job ~= nil and PlayerData.job.name == 'vigne' and (GetGameTimer() - GUI.Time) > 300 then
				if CurrentAction == 'winogrono_harvest' then
