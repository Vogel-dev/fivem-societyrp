...
Citizen.CreateThread(function()
	...
	while true do
		...
		if phone == true then
			ESX.ShowNotification('Wciśnij ~b~ "Up arrow" ~s~ to switch to ~y~ selfie ~s~ New message from.')
			ESX.ShowNotification('Wciśnij ~b~"ESC" ~s~or ~b~"ESPACE" ~s~to exit ~r~quitter ~s~mode ~y~selfie~s~.')
			ESX.ShowNotification('Wciśnij ~b~"Left Click" ~s~to ~g~take ~s~a ~y~selfie~s~.')
