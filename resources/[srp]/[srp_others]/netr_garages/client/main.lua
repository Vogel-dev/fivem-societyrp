...
Citizen.CreateThread(function()
	for k,v in pairs(Config.Garages) do
		...
		AddTextComponentString("Parking pojazdów")
		...
		if CurrentAction ~= nil then
			...
			if IsPedInAnyVehicle(playerPed) then
				if(GetVehicleEngineHealth(GetVehiclePedIsIn(playerPed)) >= 950.0) then
					...
					DisplayHelpText("~r~Aby schować ten pojazd musisz go najpierw naprawić!")
				end
			else
			...
			if IsControlJustPressed(0, Keys['E']) then
								...
								if(GetVehicleEngineHealth(GetVehiclePedIsIn(playerPed)) >= 950.0) then
									...
									ESX.TriggerServerCallback('netr_garages:checkIfVehicleIsOwned', function (owned)
										if owned ~= nil then                    
											...
											DisplayHelpText("Ten pojazd nie należy do Ciebie")
										end
									end, vehicleProps.plate)
								else
									ESX.ShowNotification("~r~Aby schować ten pojazd musisz go najpierw naprawić.")
								end
							else
								SendNUIMessage({
									clearme = true
				...
				elseif CurrentAction == 'impound_menu' and delay <= 0 then
					...
					TriggerEvent('esx:showNotification', 'Wróć za ~b~' .. delay .. '~w~ minut')
				end
				CurrentAction = nil
			end
		end
...
function ReturnVehicleMenu()
	ESX.TriggerServerCallback('netr_garages:getOwnedVehiclesOut', function (vehicles)
		...
		for _,v in pairs(vehicles) do
			...
			table.insert(elements, {label = vehicleName..': Odbierz' , value = v})
		end
		ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'spawn_vehicle',
		{
			title    = 'Odbierz pojazd z parkingu',
			align    = 'top-left',
			elements = elements,
		},
		function(data, menu)
